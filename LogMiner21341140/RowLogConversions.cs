...
namespace LogMiner
{
    ...
    {
        public List<string> ParseRowLogContents(string rowlogcontentzero, List<MetaData> metadata)
        {
            ...
            if (rowlogcontentzero.StartsWith("3"))
            {
                ...
                {
                    result.Add(Conversions.Converter(rowlogcontentzero.Substring(cont, metadata[i].Len * 2), metadata[i].Type));
                ...
                {
                    delimiters.Add(((Conversions.ToSmallInt(vlengthColumns.Substring(cont, 4)) * 2)) - (vLengthOffset + 12));
                    cont = cont + 4;
                }
                ...
                for (var i = 0; i < vlengthn; i++)
                {
                    if (i != vlengthn - 1)
                    {
                        ...
                        var toAdd = Conversions.Converter(vlengthColumns.Substring(delimiters[i], delimiters[i + 1] - (delimiters[i])), metadata.ElementAt(metadata.Count - vlengthn + i).Type);
                        result.Add(toAdd);
                    }
                    else
                    {
                        var toAdd = Conversions.Converter(vlengthColumns.Substring(delimiters[i]), metadata.ElementAt(metadata.Count - 1).Type);
                        result.Add(toAdd);
                    }
                }
            }
            else
            {
                ...
                {
                    result.Add(Conversions.Converter(rowlogcontentzero.Substring(n, metadata.ElementAt(i).Len * 2), metadata.ElementAt(i).Type));
